...
Вы можете выполнить сканирование на предмет наличия слушателей в нескольких похожих каталогах, используя символ `*` в качестве подстановочного знака:
...
Подписчики событий – это классы, которые могут подписываться на несколько событий, что позволяет вам определять несколько обработчиков событий в одном классе. Подписчики должны определить метод `subscribe`, который получает экземпляр диспетчера событий. Вы можете вызвать метод `listen` данного диспетчера для регистрации слушателей событий:
