...
> [!WARNING]
> Перед отправкой уведомлений в очередь вы должны настроить и запустить [обработчик очереди](/docs/{{version}}/queues#running-the-queue-worker).
...
> [!NOTE]
> Не используйте избыточные отступы при написании писем Markdown. Согласно стандартам Markdown, парсеры Markdown будут отображать отступы в виде блоков кода.
...
Когда уведомление сохраняется в базе данных вашего приложения, столбец `type` будет установлен на имя класса уведомления по умолчанию, а столбец `read_at` будет `null`. Однако вы можете настроить это поведение, определив методы `databaseType` и `initialDatabaseReadAtValue` в вашем классе уведомления:
...
/**
 * Получить начальное значение для столбца «read_at».
 */
...
По умолчанию хук прослушивает все уведомления. Чтобы указать типы уведомлений, которые вы хотите прослушивать, вы можете предоставить строку или массив типов для `useEchoNotification`:
...
Вы также можете указать форму данных полезной нагрузки уведомления, что обеспечивает большую безопасность типов и удобство редактирования:
