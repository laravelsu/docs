...
Для семантического поиска на основе ИИ, который сопоставляет результаты по *смыслу*, а не по точным ключевым словам, метод построителя запросов `whereVectorSimilarTo` использует векторные представления, сохранённые в PostgreSQL с расширением `pgvector` или MariaDB. Например, запрос "best wineries in Napa Valley" может найти статью "Top Vineyards to Visit", даже если слова не совпадают. Для векторного поиска требуется PostgreSQL с расширением `pgvector` либо MariaDB 11.7 или новее, а также [Laravel AI SDK](/docs/{{version}}/ai-sdk).
...
Для приложений, которым нужен трейт `Searchable`, автоматически поддерживающий поисковые индексы Eloquent-моделей в актуальном состоянии, [Laravel Scout](/docs/{{version}}/scout) предлагает как встроенный движок базы данных, так и драйверы для сторонних сервисов вроде Algolia, Meilisearch, Typesense и Turbopuffer.
...
Полнотекстовый поиск опирается на ключевые слова: слова из запроса должны присутствовать в данных в той или иной форме. Семантический поиск использует принципиально другой подход: сгенерированные ИИ векторные представления передают *смысл* текста в виде массивов чисел, после чего находятся результаты, наиболее близкие к запросу по смыслу. Например, запрос "best wineries in Napa Valley" может найти статью "Top Vineyards to Visit", даже если слова совсем не пересекаются.

Базовый процесс векторного поиска выглядит так: сгенерировать векторное представление (числовой массив) для каждого фрагмента содержимого и сохранить его вместе с данными, затем во время поиска сгенерировать такое представление для пользовательского запроса и найти ближайшие к нему сохранённые представления в векторном пространстве.

> [!NOTE]
> Векторный поиск требует [Laravel AI SDK](/docs/{{version}}/ai-sdk) и поддерживается PostgreSQL (требуется расширение `pgvector`), MariaDB 11.7 или новее и MongoDB (требуется [пакет Laravel MongoDB](https://laravel.com/docs/13.x/mongodb)). Во всех базах данных PostgreSQL в [Laravel Cloud](https://laravel.com/cloud) расширение `pgvector` уже установлено.

<a name="generating-embeddings"></a>
### Генерация векторных представлений

Векторное представление - многомерный числовой массив (обычно из сотен или тысяч чисел), передающий семантический смысл фрагмента текста. Его можно сгенерировать для строки с помощью метода `toEmbeddings`, доступного в классе Laravel `Stringable`:
...
Чтобы сгенерировать векторные представления сразу для нескольких входных значений, что эффективнее обработки по одному благодаря единственному вызову API провайдера, используйте класс `Embeddings`:
...
Подробнее о настройке провайдеров, размерностях векторов и кешировании смотрите в [документации AI SDK](/docs/{{version}}/ai-sdk#embeddings).

<a name="storing-and-indexing-vectors"></a>
### Хранение и индексирование векторов

Чтобы хранить векторные представления, определите в миграции столбец `vector`, указав размерность, соответствующую выводу используемого провайдера (например, 1536 для модели OpenAI `text-embedding-3-small`). Также следует вызвать `index` для столбца, чтобы создать HNSW-индекс (Hierarchical Navigable Small World), который значительно ускоряет поиск по сходству в больших наборах данных:
...
В Eloquent-модели используйте приведение `AsVector`, чтобы Laravel автоматически преобразовывал массивы PHP во векторный формат базы данных и обратно:
...
Подробнее о векторных столбцах и индексах смотрите в [документации по миграциям](/docs/{{version}}/migrations#available-column-types).

<a name="querying-by-similarity"></a>
### Запросы по сходству

После сохранения векторных представлений содержимого можно искать похожие записи с помощью метода `whereVectorSimilarTo`. Метод сравнивает заданное представление с сохранёнными векторами по косинусному сходству, отфильтровывает результаты ниже порога `minSimilarity` и автоматически сортирует их по релевантности, начиная с наиболее похожих. Порог должен быть значением от `0.0` до `1.0`, где `1.0` означает идентичные векторы:
...
Для удобства, если вместо массива векторного представления передана обычная строка, Laravel автоматически сгенерирует представление через настроенного провайдера. Поэтому пользовательский поисковый запрос можно передавать напрямую:
...
Реранжирование - это метод, при котором модель ИИ переупорядочивает набор результатов по их семантической релевантности заданному запросу. В отличие от векторного поиска, требующего заранее вычислить и сохранить векторные представления, реранжирование работает с любой коллекцией текста: оно принимает исходное содержимое и запрос и возвращает элементы, отсортированные по релевантности.

Реранжирование особенно эффективно как второй этап после быстрого первичного отбора. Например, с помощью полнотекстового поиска можно быстро сократить тысячи записей до 50 кандидатов, а затем применить реранжирование, чтобы поднять наиболее релевантные результаты. Такой двухэтапный подход сочетает скорость и семантическую точность.
...
Комбинируйте векторное сходство со стандартными условиями `where`, чтобы ограничить семантический поиск подмножеством записей. Это полезно, когда нужен смысловой поиск, но результаты должны быть ограничены владельцем, категорией или любым другим атрибутом:
