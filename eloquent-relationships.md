---
git: f966ff9d9aa8b62bceac367d3c14854c39a3bc10
---
...
Вы также можете использовать метод `one` для преобразования отношений `HasManyThrough` в отношения `HasOneThrough`:
...
Теперь, когда мы изучили структуру таблицы для отношения, давайте определим отношение в модели `Application`:
...
Или, если соответствующие отношения уже определены для всех моделей, участвующих в отношении, вы можете легко определить отношение «многие-через-отношение», вызвав метод `through` и указав имена этих отношений. Например, если у модели `Application` есть отношение `environments`, а у модели `Environment` есть отношение `deployments`, вы можете определить отношение «многие-через-отношение», соединяющее приложение и деплойменты, следующим образом:
...
Хотя таблица модели `Deployment` не содержит столбца `application_id`, отношение `hasManyThrough` обеспечивает доступ к `deployments` приложения через `$application->deployments`. Чтобы получить эти модели, Eloquent проверяет столбец `application_id` в сводной таблице модели `Environment`. После нахождения соответствующих идентификаторов `environments` они используются для запроса таблицы модели `Deployment`.
...
class Project extends Model
{
    public function deployments(): HasManyThrough
    {
        return $this->hasManyThrough(
            Deployment::class,
            Environment::class,
            'application_id', // Внешний ключ в таблице `environments` ...
            'environment_id', // Внешний ключ в таблице `deployments` ...
            'id', // Локальный ключ в таблице `applications` ...
...
Обычно добавляются дополнительные методы к моделям, которые ограничивают отношения. Например, вы можете добавить метод `featuredPosts` к модели `User`, которая ограничивает более широкое отношение `posts` с дополнительным ограничением `where`:
...
Однако, если вы попытаетесь создать модель с помощью метода `featuredPosts`, ее атрибут `featured` не будет установлен в `true`. Если вы хотите создать модели с помощью методов отношений и также указать атрибуты, которые должны быть добавлены ко всем моделям, созданным с помощью этого отношения, вы можете использовать метод `withAttributes` при построении запроса отношения:
...
Метод `withAttributes` добавит условия `where` к запросу, используя заданные атрибуты, а также добавит заданные атрибуты к любым моделям, созданным с помощью метода отношения:
...
Чтобы указать методу `withAttributes` не добавлять условия `where` в запрос, можно задать аргументу `asConditions` значение `false`:
...
`wherePivot` добавляет ограничение предложения where к запросу, но не добавляет указанное значение при создании новых моделей через определенную связь. Если вам нужно и запросить, и создать связи с определенным значением pivot, вы можете использовать метод `withPivotValue`:
...
Если вы определили отношение «многие-ко-многим», которое использует явную сводную модель, и эта сводная модель имеет автоинкрементный первичный ключ, то вы должны убедиться, что ваш класс явной сводной модели использует атрибут `Table` со значением `true` для `incrementing`:
...
#### Запросы на существование отношений «многие ко многим»

Метод `whereAttachedTo` может использоваться для запроса моделей, имеющих привязку «многие ко многим» к модели или коллекции моделей:

```php
$users = User::whereAttachedTo($role)->get();
```

Вы также можете предоставить экземпляр [collection](/docs/{{version}}/eloquent-collections) методу `whereAttachedTo`. При этом Laravel извлечет модели, которые прикреплены к любой из моделей в коллекции:
...
Вы можете использовать «точечную нотацию» для выполнения запроса к вложенным отношениям. Например, следующий запрос будет извлекать все посты, у которых нет комментариев, а также записи, в которых есть комментарии, и ни один из комментариев не принадлежит заблокированным пользователям:
...
Чтобы узнать о существовании полиморфных отношений «один-к-одному», вы можете использовать методы `whereHasMorph` и `whereDoesntHaveMorph`. Эти методы принимают имя отношения в качестве своего первого аргумента. Затем методы принимают имена связанных моделей, которые вы хотите включить в запрос. Наконец, вы можете предоставить замыкание, которое ограничивает запрос отношения:
...
Если вы хотите загрузить полиморфное отношение «один-к-одному», а также связанные счетчики моделей для различных сущностей, которые могут быть возвращены этим отношением, то вы можете использовать метод `with` в сочетании с отношениями `morphTo` - метод `morphWithCount`.

В этом примере предположим, что модели `Photo` и `Post` могут создавать модели `ActivityFeed`. Предположим, что модель `ActivityFeed` определяет полиморфное отношение «один-к-одному» с именем `parentable`, которое позволяет нам получить родительскую модель `Photo` или `Post` для переданного экземпляра `ActivityFeed`. Кроме того, предположим, что модели `Photo` имеют много моделей `Tag`, а модели `Post` имеют много моделей `Comment`.
...
Если вы хотите жадно загрузить полиморфное отношение «один-к-одному», Eloquent выполнит несколько запросов для получения каждого типа связанной модели. Вы можете добавить дополнительные ограничения к каждому из этих запросов, используя метод `constrain` полиморфного отношения «один-к-одному»:
...
Если вы хотите жадно загрузить полиморфное отношение «один-к-одному», а также вложенные отношения для различных сущностей, которые могут быть возвращены этим отношением, вы можете использовать метод `loadMorph`.

Этот метод принимает имя полиморфного отношения «один-к-одному» в качестве своего первого аргумента и массив пар модель / отношение в качестве второго аргумента. Чтобы проиллюстрировать этот метод, давайте рассмотрим следующую модель:
...
### Автоматическая жадная загрузка

> [!WARNING]
> Эта функция в настоящее время находится в стадии бета-тестирования для сбора отзывов сообщества. Поведение и функциональность этой функции могут измениться даже в релизах патчей.

Во многих случаях Laravel может автоматически загружать отношения, к которым вы обращаетесь. Чтобы включить автоматическую загрузку, вам следует вызвать метод `Model::automaticallyEagerLoadRelationships` в методе `boot` вашего приложения `AppServiceProvider`:
...
Когда эта функция включена, Laravel попытается автоматически загрузить любые отношения, к которым вы обращаетесь, но которые не были загружены ранее. Например, рассмотрим следующий сценарий:
...
Обычно код выше выполняет запрос для каждого пользователя, чтобы получить его посты, а также запрос для каждого поста, чтобы получить его комментарии. Однако, когда функция `automaticallyEagerLoadRelationships` включена, Laravel автоматически выполнит [жадную пост-загрузку](#lazy-eager-loading) постов для всех пользователей в коллекции пользователей, когда вы пытаетесь получить доступ к постам любого из извлеченных пользователей. Аналогично, когда вы пытаетесь получить доступ к комментариям для любого извлеченного поста, все комментарии будут "жадно пост-загружены" для всех постов, которые были изначально получены.

Если вы не хотите включать автоматическую жадную загрузку глобально, вы все равно можете включить эту функцию для одного экземпляра коллекции Eloquent, вызвав метод `withRelationshipAutoloading` для коллекции:
...
#### Транзакционные pivot-операции

Каждая из рассмотренных выше pivot-операций также имеет вариант `OrFail` (`attachOrFail`, `detachOrFail`, `syncOrFail`, `syncWithoutDetachingOrFail` и `toggleOrFail`), который оборачивает операцию в транзакцию базы данных, чтобы все изменения автоматически откатывались при возникновении исключения:
...
Например, когда модель `Comment` обновляется, то вы можете автоматически «затронуть» временную метку `updated_at` родительской модели `Post`, чтобы она была установлена на текущую дату и время. Для этого вы можете использовать атрибут `Touches` в дочерней модели, содержащий имена отношений, для которых должны обновляться временные метки `updated_at` при обновлении дочерней модели:
