---
git: ab2f0371c0501e33e5f4176c730355315c57475b
---
...
Самый простой способ начать разработку нового пакета Laravel - использовать официальный [каркас пакета Laravel](https://github.com/laravel/package-skeleton). Каркас предоставляет все необходимое для создания пакета Laravel, включая сервис-провайдер, тестирование с помощью Pest, статический анализ через Larastan, форматирование кода через Pint и workbench-приложение для сквозной разработки пакета. Вы можете создать новый пакет с помощью команды `package` из [CLI установщика Laravel](/docs/{{version}}/installation#creating-a-laravel-project):

```shell
laravel package my-package
```

Интерактивный скрипт конфигурации персонализирует каркас под ваш пакет, настроив пространство имен, сервис-провайдер и только те возможности, которые вам нужны, например файлы конфигурации, маршруты, шаблоны, переводы, миграции, ресурсы, команды и фасад.
...
Файл `bootstrap/providers.php` содержит список сервис-провайдеров, которые должны быть загружены Laravel. Однако вместо того, чтобы требовать от пользователей вручную добавлять ваш сервис-провайдер в этот список, вы можете определить провайдер в разделе `extra` файла `composer.json` вашего пакета, чтобы он автоматически загружался Laravel. Помимо сервис-провайдеров, вы также можете перечислить любые [фасады](/docs/{{version}}/facades), которые вы хотели бы зарегистрировать:
...
Если ваш пакет содержит [миграции базы данных](/docs/{{version}}/migrations), вы можете использовать метод `publishesMigrations`, чтобы сообщить Laravel, что указанный каталог или файл содержит миграции. Когда Laravel публикует миграции, он автоматически обновляет временные метки в их имени файла, отражая текущую дату и время:
...
Команда Laravel [reload](/docs/{{version}}/deployment#reloading-services) завершает любые запущенные сервисы, чтобы системный монитор процессов мог автоматически запустить их заново. Используя метод `reloads`, вы можете зарегистрировать собственные команды Artisan вашего пакета, которые должны вызываться при выполнении команды `reload`:
...
Теперь, когда пользователи вашего пакета выполнят команду `vendor:publish`, ваши ресурсы будут скопированы в указанное место публикации. Поскольку пользователям обычно требуется перезаписывать ресурсы каждый раз при обновлении пакета, они могут использовать флаг `--force`:
...
Ваши пользователи также могут публиковать все публикуемые файлы, определенные поставщиком услуг вашего пакета, с помощью флага `--provider`:
