...
Чтобы сделать это с помощью Precognition, вам следует вызвать метод `validate`, передав имена полей, которые вы хотите проверить, ключу конфигурации `only`. Вы можете обработать результат проверки с помощью обратных вызовов `onSuccess` или `onValidationError`:
