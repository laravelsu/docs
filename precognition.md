---
git: 8561390252a1b5632e0d15adc160b3519fd21f0b
---
...
Для этого в Precognition необходимо вызвать метод `validate`, передав имена полей, которые вы хотите проверить, в конфигурационный ключ `only`. Вы можете обработать результат проверки с помощью обратных вызовов `onSuccess` или `onValidationError`:
...
Для этого с помощью Precognition необходимо вызвать метод `validate`, передав имена полей, которые требуется проверить, в конфигурационный ключ `only`. Результат проверки можно обработать с помощью обратных вызовов `onSuccess` или `onValidationError`:
