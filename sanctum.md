---
git: 87325d4d2bcfa47d1ee57fb65f69823d11ffff3a
---
...
Чтобы помочь вам настроить ваши домены с отслеживанием состояния (stateful domains), Sanctum предоставляет две вспомогательные функции, которые можно добавить в конфигурацию. Во-первых, `Sanctum::currentApplicationUrlWithPort()` вернет текущий URL-адрес приложения из переменной среды `APP_URL`, а `Sanctum::currentRequestHost()` добавляет заполнитель в список доменов с отслеживанием состояния. Во время выполнения этот заполнитель заменяется на хост из текущего запроса, так что все запросы с тем же доменом считаются stateful.
...
Кроме того, вы должны включить опции `withCredentials` и `withXSRFToken` в глобальном экземпляре `axios` вашего приложения. Это можно сделать в файле `resources/js/app.js`. Если вы не используете Axios для выполнения HTTP-запросов из вашего фронтенда, вы должны выполнить аналогичную настройку для вашего собственного HTTP-клиента:
...
Поскольку этот подход к аутентификации SPA основан на сессиях, вы можете использовать стандартные службы аутентификации Laravel, включая функциональность ["запомнить меня"](/docs/{{version}}/authentication#remembering-users).
