...
[Laravel Tinker](https://github.com/laravel/tinker) — это мощный REPL для фреймворка Laravel, работающий на основе пакета [PsySH](https://github.com/bobthecow/psysh).
...
В дополнение к командам Artisan, вы можете создавать пользовательские команды. Команды обычно хранятся в каталоге `app/Console/Commands`; однако вы можете выбрать другое месторасположение, если вы дадите команду [сканировать другие каталоги на наличие команд Artisan](#registering-commands).
...
Когда вы помечаете команду как `Isolatable`, Laravel автоматически делает параметр `--isolated` доступным для команды без необходимости явно определять его в параметрах команды. Когда команда вызывается с этой опцией, Laravel гарантирует, что никакие другие экземпляры этой команды в данный момент не выполняются. Laravel достигает этого, пытаясь получить блокировку с использованием кэш-драйвера по умолчанию вашего приложения. Если другие экземпляры команды выполняются, команда не будет выполнена; однако команда все равно завершится с кодом успешного завершения:
...
При запуске этой команды аргументы `user` могут передаваться по порядку в командную строку. Например, следующая команда установит значение `user` как `1` и `2`:
...
> [!NOTE]
> [Laravel Prompts](/docs/{{version}}/prompts) - это PHP-пакет для добавления красивых и удобных форм в консольное приложение с такими привычными для браузера возможностями, как заполнители и проверка данных.
...
Чтобы вывести в консоль, используйте методы `line`, `newLine`, `info`, `comment`, `question`, `warn`, `alert` и `error`. Каждый из этих методов будет использовать соответствующие ANSI-цвета. Например, давайте покажем пользователю некоторую общую информацию. Обычно метод `info` отображается в консоли в виде зеленого текста:
...
Для управления процессами команда `dev` использует npm-пакет `@laravel/multiplex`, предоставляя каждому процессу отдельную вкладку с прокручиваемым выводом и возможностью поиска. Каждый процесс имеет метку и цветовое выделение, чтобы их было легко различать. Если процесс аварийно завершится, он будет автоматически перезапущен. При выходе весь вывод записывается обратно в терминал, поэтому ничего не теряется.

> [!NOTE]
> Для команды `dev` требуется Node 22.13 или более поздней версии. В Windows используется резервный npm-пакет `concurrently`, а интерфейс со вкладками недоступен.
...
#### Перезапуск завершившихся с ошибкой процессов

Если процесс аварийно завершится, Laravel будет перезапускать его после небольшой задержки до пяти раз, а затем отметит как завершившийся с ошибкой. Процесс, завершившийся менее чем через секунду после запуска, не перезапускается, поскольку, скорее всего, он изначально не смог успешно запуститься. Ручной перезапуск процесса клавишей `r` сбрасывает счетчик.

Это поведение можно отключить для отдельного запуска с помощью опции `--no-restart`:

```shell
php artisan dev --no-restart
```

Или отключить его для всего приложения с помощью метода `disableAutoRestart`:
