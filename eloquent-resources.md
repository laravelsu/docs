...
Для удобства вы можете использовать метод модели `toResource`, который будет использовать соглашения фреймворка для автоматического обнаружения базового ресурса модели:
...
Или, для удобства, вы можете использовать метод `toResourceCollection` коллекции Eloquent, который будет использовать соглашения фреймворка для автоматического обнаружения базовой коллекции ресурсов модели:
...
По умолчанию коллекции ресурсов не позволяют добавлять пользовательские метаданные, которые могут потребоваться при возвращении с вашей коллекцией. Если вы хотите получить больший контроль над ответом коллекции ресурса, то вы можете создать выделенный ресурс для представления коллекции:
...
В то время как ресурсы преобразуют одну модель в массив, коллекции ресурса преобразуют коллекцию моделей в массив. Однако, необязательно определять класс коллекции ресурса для каждой из ваших моделей, поскольку все коллекции моделей Eloquent предоставляют метод `toResourceCollection` для генерации коллекции ресурсов `ad-hoc` на лету:
...
Или, для удобства, вы можете использовать метод `toResourceCollection` пагинатора, который будет использовать соглашения фреймворка для автоматического обнаружения базовой коллекции ресурсов постраничной модели:
