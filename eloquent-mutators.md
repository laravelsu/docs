...
Если ваша модель Eloquent имеет столбец `uuid` или `ulid` [бинарного типа](/docs/{{version}}/migrations#column-method-binary) в дополнение к автоматически увеличивающемуся столбцу ID модели, вы можете использовать приведение `AsBinary`, чтобы автоматически преобразовывать значение в его бинарное представление и обратно:
...
После определения приведения в модели вы можете задать значение атрибута UUID / ULID как экземпляр объекта или строку. Eloquent автоматически преобразует значение в его бинарное представление. При получении значения атрибута вы всегда будете получать обычную текстовую строку:
...
Как вы, возможно, знаете, для шифрования строк Laravel использует значение `key`, указанное в конфигурационном файле `app` вашего приложения. Обычно это значение соответствует значению переменной окружения `APP_KEY`. Если вам необходимо изменить ключ шифрования вашего приложения, вы можете [сделать это плавно](/docs/{{version}}/encryption#gracefully-rotating-encryption-keys).
