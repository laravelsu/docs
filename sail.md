...
По сути, Sail - это файл `compose.yaml`, который хранится в корне вашего проекта и набор скриптов `sail`, при помощи которых можно управлять docker-контейнерами, определёнными в `compose.yaml`.
...
После установки Sail вы можете запустить Artisan-команду `sail:install`. Эта команда опубликует файл Sail `compose.yaml` в корень вашего приложения и изменит файл `.env`, добавив необходимые переменные окружения для подключения к Docker-сервисам::
...
> [!WARNING]
>Если вы используете Docker Desktop для Linux, выполните следующую команду, чтобы использовать контекст Docker по умолчанию: `docker context use default`. Кроме того, если вы столкнетесь с ошибками прав доступа к файлам внутри контейнеров, вам может потребоваться установить переменную среды `SUPERVISOR_PHP_USER` в значение `root`.
...
Файл `compose.yaml` Laravel Sail определяет различные контейнеры Docker, которые работают вместе, чтобы помочь вам создавать приложения Laravel. Чтобы узнать, что это за контейнеры - обратитесь к записи `services` вашего файла `compose.yaml`. Контейнер `laravel.test` - это основной контейнер, который будет обслуживать ваше приложение.

Перед запуском Sail убедитесь, что на вашем локальном компьютере не работают другие веб-серверы или базы данных. Чтобы запустить все контейнеры Docker, определенные в файле `compose.yaml` вашего приложения, вы должны выполнить команду `up`:
...
Как вы могли заметить, в файле `compose.yaml` есть описание контейнера MySQL. Этот контейнер использует [том Docker](https://docs.docker.com/storage/volumes/), чтобы данные, хранящиеся в вашей базе данных, сохранялись даже при остановке и перезапуске ваших контейнеров.
...
В файле `compose.yaml` также есть описание контейнера [Redis](https://redis.io). Этот контейнер использует [том Docker](https://docs.docker.com/storage/volumes/), чтобы данные, хранящиеся в вашем экземпляре Redis, сохранялись даже при остановке и перезапуске ваших контейнеров. После того как вы запустили свои контейнеры, вы можете подключиться к экземпляру Redis в своем приложении, установив для переменной среды `REDIS_HOST` в файле` .env` вашего приложения значение `redis`.
...
Если вы решите установить службу Valkey при установке Sail, файл `compose.yaml` вашего приложения будет содержать запись для [Valkey](https://valkey.io/). В этом контейнере используется [том Docker](https://docs.docker.com/storage/volumes/), поэтому данные, хранящиеся в вашем экземпляре Valkey, сохраняются даже при остановке и перезапуске ваших контейнеров. Вы можете подключиться к этому контейнеру в своем приложении, установив для переменной среды `REDIS_HOST` в файле `.env` вашего приложения значение `valkey`.

Чтобы подключиться к базе данных Valkey вашего приложения с вашего локального компьютера, вы можете использовать графическое приложение для управления базой данных, такое как [TablePlus](https://tableplus.com). По умолчанию база данных Valkey доступна по порту `localhost` 6379.

<a name="meilisearch"></a>
### Meilisearch

Если вы решили установить службу [Meilisearch](https://www.meilisearch.com) при установке Sail, файл `compose.yaml` вашего приложения будет содержать запись для этой мощной поисковой системы, интегрированной с [Laravel Scout](/docs/{{version}}/scout). После запуска контейнеров вы можете подключиться к экземпляру Meilisearch в вашем приложении, установив для переменной среды `MEILISEARCH_HOST` значение `http://meilisearch:7700`.
...
Если вы решили установить службу [Typesense](https://typesense.org) при установке Sail, ваш файл `compose.yaml` вашего приложения будет содержать запись для этого быстрого и открытого поискового движка, который интегрирован с [Laravel Scout](/docs/{{version}}/scout#typesense). После запуска ваших контейнеров, вы можете подключиться к экземпляру Typesense в вашем приложении, установив следующие переменные окружения:
...
Если вы планируете использовать Amazon S3 для хранения файлов при запуске приложения в производственной среде, вы можете установить службу [RustFS](https://rustfs.com) при установке Sail. RustFS предоставляет совместимый с S3 API, который вы можете использовать для локальной разработки с помощью драйвера хранилища файлов Laravel s3, не создавая «тестовых» сегментов хранилища в производственной среде S3. Если вы выберете установку RustFS при установке Sail, раздел конфигурации RustFS будет добавлен в файл `compose.yaml` вашего приложения.

По умолчанию файл конфигурации приложения `filesystems` уже содержит конфигурацию диска для диска `s3`. Помимо использования этого диска для взаимодействия с Amazon S3, вы можете использовать его для взаимодействия с любой S3-совместимой службой хранения файлов, такой как RustFS, путем простого изменения связанных переменных среды, которые управляют его конфигурацией. Например, при использовании RustFS конфигурация переменной среды вашей файловой системы должна быть определена следующим образом:
...
Затем убедитесь, что служба `laravel.test` в файле `compose.yaml` вашего приложения имеет запись `depends_on` для `selenium`:
...
Кроме того, вы можете захотеть обновить имя `image`, чтобы оно отражало версию PHP, используемую приложением. Этот параметр также определен в файле `compose.yaml` приложения:

```yaml
image: sail-8.2/app
```

После обновления файла `compose.yaml` вашего приложения вы должны обновить образы контейнеров:
...
Для версий Docker старше 20.10 не поддерживается `host.docker.internal` в Linux, и вам придется вручную определить IP-адрес хоста. Для этого настройте статический IP-адрес для вашего контейнера, определив собственную сеть в файле `compose.yaml`:
...
```shell
# Запускаем команду Artisan без Xdebug...
sail artisan migrate

# Запускаем команду Artisan с помощью Xdebug...
sail debug migrate
```
...
После выполнения этой команды файлы Dockerfiles и другие файлы конфигурации, используемые Laravel Sail, будут помещены в каталог `docker` в корневом каталоге вашего приложения. После настройки вашей установки Sail вы можете изменить имя образа для контейнера приложения в файле `compose.yaml` вашего приложения. После этого пересоберите контейнеры приложения с помощью команды `build`. Назначение уникального имени образу приложения особенно важно, если вы используете Sail для разработки нескольких приложений Laravel на одной машине:
