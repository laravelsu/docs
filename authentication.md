---
git: f0681c0f01e3b39b7b5f8208af1dd1136b329ef1
---
...
Метод `updateRememberToken` обновляет поле `remember_token` пользователя `$user` значением нового `$token`. Новый ключ может быть свежим ключом, как назначенным при успешной попытке входа "запомнить меня", так и при выходе пользователя.
