...
При использовании драйвера сессии `database` вам необходимо убедиться, что у вас есть таблица базы данных, содержащая данные сеанса. Обычно это включено в стандартный файл Laravel `0001_01_01_000000_create_users_table.php` [миграции базы данных](/docs/{{version}}/migrations); однако, если по какой-либо причине у вас нет таблицы `sessions`, вы можете использовать Artisan-команду `make:session-table` для создания этой миграции:
...
Кеш сессии Laravel предоставляет удобный способ кешировать данные, ограниченные отдельной пользовательской сессией. В отличие от глобального кеша приложения, данные кеша сессии автоматически изолируются для каждой сессии и очищаются, когда сессия истекает или уничтожается. Кеш сессии поддерживает все привычные [методы кеша Laravel](/docs/{{version}}/cache), такие как `get`, `put`, `remember`, `forget` и другие, но в области текущей сессии.

Кеш сессии отлично подходит для хранения временных пользовательских данных, которые нужно сохранять между несколькими запросами в рамках одной сессии, но не нужно хранить постоянно. Это могут быть данные формы, временные вычисления, ответы API или любые другие эфемерные данные, которые должны быть привязаны к конкретной сессии пользователя.

Вы можете получить доступ к кешу сессии через метод `cache` сессии:
...
Дополнительную информацию о методах кеша Laravel можно найти в [документации по кешу](/docs/{{version}}/cache).

<a name="session-blocking"></a>
## Блокировка сессии

> [!WARNING]
> Чтобы использовать блокировку сессии, ваше приложение должно использовать драйвер кеша, поддерживающий [атомарные блокировки](/docs/{{version}}/cache#atomic-locks). В настоящее время этими драйверами кеширования являются `memcached`, `dynamodb`, `redis`, `mongodb` (включены в официальный пакет `mongodb/laravel-mongodb`),  `database`, `file` и `array`. Кроме того, вы не можете использовать драйвер сессии `cookie`.
...
Поскольку Laravel не содержит каталога для хранения ваших расширений. Вы можете разместить их где угодно. В этом примере мы создали каталог `Extensions` для размещения `MongoSessionHandler`.

Поскольку цель этих методов не совсем понятна, вот краткий обзор назначения каждого метода:
...
class SessionServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        Session::extend('mongo', function (Application $app) {
            // Возвращаем реализацию SessionHandlerInterface...
