...
Драйвер `local` взаимодействует с файлами, хранящимися локально на сервере, на котором запущено приложение Laravel, в то время как драйвер хранилища `sftp` используется для FTP на основе ключей SSH. Драйвер `s3` используется для записи в службу облачного хранилища Amazon S3.
...
При использовании драйвера `local` все операции с файлами выполняются относительно корневого каталога, определенного в файле конфигурации `filesystems`. По умолчанию это значение задано каталогом `storage/app/private`. Следовательно, следующий метод запишет файл в `storage/app/private/example.txt`:
...
Если ваш `public` диск использует драйвер `local` и вы хотите сделать эти файлы доступными из Интернета, вам следует создать символическую ссылку из исходного каталога `storage/app/public` в целевой каталог `public/storage`:
...
Метод `files` возвращает массив всех файлов в указанном каталоге. Если вы хотите получить список всех файлов в указанном каталоге, включая подкаталоги, вы можете использовать метод `allFiles`:
...
Метод `directories` возвращает массив всех каталогов в заданном каталоге. Если вы хотите получить список всех каталогов в заданном каталоге, включая подкаталоги, вы можете использовать метод `allDirectories`:
