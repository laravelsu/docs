...
Вы также можете создать коллекцию, используя методы [make](#method-make) и [fromJson](#method-fromjson).
...
Метод `chunkBy` разбивает коллекцию на несколько меньших коллекций, группируя соседние элементы с одинаковым значением заданного ключа или результата замыкания. Например, можно сгруппировать соседние товары с одним родителем:

```php
$chunks = $products->chunkBy('parent');
```

В отличие от метода `groupBy`, несоседние элементы с одинаковым значением попадают в разные группы:
...
Метод `collapse` сворачивает коллекцию массивов или коллекций в единую плоскую коллекцию:
...
Метод `collapseWithKeys` сглаживает коллекцию массивов или коллекций в одну коллекцию, сохраняя исходные ключи нетронутыми. Если коллекция уже плоская, он вернет пустую коллекцию:
...
Обратный вызов должен быть функцией сравнения, которая возвращает целое число меньше, равное или больше нуля. Дополнительную информацию можно найти в документации PHP по [array_diff_uassoc](https://www.php.net/array_diff_uassoc#refsect1-function.array-diff-uassoc-parameters), функции PHP, которую внутренне использует метод diffAssocUsing.
...
Статический метод `fromJson` создает новый экземпляр коллекции путем декодирования заданной строки JSON с помощью функции PHP `json_decode`:
...
Метод `intersectUsing` удаляет любые значения из исходной коллекции, которые отсутствуют в заданном массиве или коллекции, используя пользовательский обратный вызов для сравнения значений. Результирующая коллекция сохранит ключи исходной коллекции:
...
Метод `intersectAssoc` сравнивает исходную коллекцию с другой коллекцией или массивом, возвращая пары ключ / значение, которые присутствуют во всех заданных коллекциях:
...
Метод `pop` удаляет и возвращает последний элемент из коллекции. Если коллекция пуста, будет возвращен `null`:
...
Метод `reduce` также передает ключи массива указанному замыканию:
...
Метод `replaceRecursive` ведет себя аналогично `replace`, но он будет повторяться в массивах и применять тот же процесс замены к внутренним значениям:
...
Метод `skipUntil` пропускает элементы из коллекции, в то время как данное замыкание возвращает `false`. Как только замыкание вернет `true`, все оставшиеся элементы коллекции будут возвращены как новая коллекция:
...
Метод `skipWhile` пропускает элементы из коллекции, в то время как замыкане возвращает `true`. Как только обратный вызов вернет `false`, все оставшиеся элементы коллекции будут возвращены как новая коллекция:
...
Метод `unless` выполнит указанное замыкание, если первый аргумент, переданный методу, не будет иметь значение `true`. Экземпляр коллекции и первый аргумент, переданный методу `unless`, будут переданы замыканию:
