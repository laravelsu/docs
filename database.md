...
### Пулированные соединения PostgreSQL

Многие управляемые провайдеры PostgreSQL предлагают пул соединений в режиме транзакций через такие сервисы, как PgBouncer или прокси соединений. Такие пулеры хорошо подходят для запросов приложения, но некоторые операции со схемой, миграции и команды обслуживания требуют прямого соединения с базой данных.

Чтобы использовать транзакционный пулер с PostgreSQL, настройте пулированное соединение как обычно и укажите параметры прямого соединения через параметр конфигурации `direct`:
...
Когда соединение PostgreSQL настроено как пулированное, Laravel автоматически включает эмулируемые подготовленные выражения для пулированного соединения. Прямое соединение наследует все параметры, которые явно не определены в конфигурации `direct`, и по умолчанию использует нативные подготовленные выражения.

Laravel автоматически использует прямое соединение для миграций, выгрузки и восстановления схемы, `db:wipe`, `db:show` и `db:table`. Команда `db` также по умолчанию использует прямое соединение, когда включен режим пула и настроено прямое соединение; вы можете передать опцию `--pooled`, чтобы подключиться к пулированному соединению:

```shell
php artisan db --pooled
```

Если вам нужно явно использовать прямое соединение в приложении, добавьте суффикс `::direct` к имени соединения:
