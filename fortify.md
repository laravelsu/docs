...
Затем, опубликуйте ресурсы Fortify с помощью Artisan-команды `fortify:install`:
...
Файл конфигурации `fortify` содержит массив конфигурации `features`. Этот массив определяет, какие серверные маршруты / функционал будет предоставлять Fortify по умолчанию. Мы рекомендуем вам включить только следующие функции, которые являются базовыми функциями аутентификации, предоставляемыми большинством приложений Laravel:
...
#### Регулирование аутентификации

По умолчанию Fortify будет регулировать попытки аутентификации с помощью посредника `EnsureLoginIsNotThrottled`. Этот посредник ограничивает попытки, уникальные для комбинации имени пользователя и IP-адреса.

Некоторым приложениям может потребоваться другой подход к регулированию попыток аутентификации, например регулирование только по IP-адресу. Таким образом, Fortify позволяет вам указать свой собственный [ограничитель скорости](/docs/{{version}}/routing#rate-limiting) с помощью параметра конфигурации `fortify.limiters.login`. Конечно, этот параметр конфигурации находится в файле конфигурации вашего приложения `config/fortify.php`.

> [!NOTE]
> Использование сочетания регулирования, [двухфакторной аутентификации](/docs/{{version}}/fortify#two-factor-authentication) и внешнего брандмауэра веб-приложений (WAF) обеспечит наиболее надежную защиту для ваших законных пользователей приложения.
...
Чтобы указать, что для маршрута или группы маршрутов требуется, чтобы пользователь подтвердил свой адрес электронной почты, то вам следует назначить маршруту встроенным в Laravel посредника `verify`. Псевдоним посредника `verified` автоматически регистрируется Laravel и служит псевдонимом для посредника `Illuminate\Auth\Middleware\EnsureEmailIsVerified`:
