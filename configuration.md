---
git: bea48ec9fd314ebb6d578854fcc7d33c0ac77108
---
...
Конфигурационные файлы позволяют настраивать такие вещи, как информация о подключении к базе данных, информация о почтовом сервере, а также другие основные параметры, например, URL-адрес приложения и ключ шифрования.
...
В качестве альтернативы Laravel предлагает метод управления режимом обслуживания, основанный на кешировании. Этот метод требует выполнения команды `php artisan down` только на одном сервере. Чтобы использовать этот подход, измените переменные режима обслуживания в файле `.env` вашего приложения. Вам следует выбрать `store` кэша, доступный для всех ваших серверов. Это обеспечит единообразное поддержание статуса режима обслуживания на всех серверах:
