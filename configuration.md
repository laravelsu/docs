...
Конфигурационные файлы позволяют настраивать такие вещи, как информация о подключении к базе данных, информация о почтовом сервере, а также другие основные параметры, например, URL приложения и ключ шифрования.
...
В качестве альтернативы Laravel предлагает метод на основе кэширования для работы в режиме обслуживания. Этот метод требует запуска команды `php artisan down` только на одном сервере. Чтобы использовать этот подход, измените переменные режима обслуживания в файле `.env` вашего приложения. Вам следует выбрать `store` кэша, доступный для всех ваших серверов. Это гарантирует, что статус режима обслуживания постоянно поддерживается на каждом сервере:
...
Поскольку режим обслуживания требует, чтобы ваше приложение простаивало несколько секунд, то рассмотрите возможность запуска своих приложений на полностью управляемой платформе, такой как [Laravel Cloud](https://cloud.laravel.com), чтобы обеспечить развертывание с нулевым временем простоя.
