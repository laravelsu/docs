...
#### Готовый к использованию агентов фреймворк

Четко сформулированные соглашения и хорошо определенная структура Laravel делают его идеальным фреймворком для [разработки с использованием ИИ](/docs/{{version}}/ai) с помощью таких инструментов, как Cursor и Claude Code. Когда вы просите ИИ-агента добавить контроллер, он точно знает, куда его поместить. Когда вам нужна новая миграция, соглашения об именовании и расположение файлов предсказуемы. Такая согласованность исключает догадки, которые часто мешают работе инструментов ИИ в более гибких средах.

Помимо организации файлов, выразительный синтаксис Laravel и исчерпывающая документация предоставляют агентам ИИ необходимый контекст для генерации точного, идиоматического кода. Такие функции, как отношения Eloquent, запросы форм и посредники, следуют шаблонам, которые агенты могут надежно понимать и воспроизводить. В результате получается сгенерированный ИИ код, который выглядит так, будто его написал опытный разработчик Laravel, а не собрал из стандартных фрагментов PHP-кода.

Чтобы узнать больше о том, почему Laravel идеально подходит для разработки с использованием ИИ, ознакомьтесь с нашей документацией по [разработке с использованием агентов](/docs/{{version}}/ai).
...
Для всесторонней и надежной поддержки Laravel обратите внимание на [PhpStorm](https://www.jetbrains.com/phpstorm/laravel/), IDE от JetBrains. Встроенная поддержка фреймворка Laravel в PhpStorm охватывает шаблоны Blade, интеллектуальное автодополнение для моделей Eloquent, маршрутов, представлений, переводов и компонентов, а также мощную генерацию кода и навигацию по проектам Laravel.
...
#### Добавление пользовательских рекомендаций по использованию ИИ

Чтобы расширить функциональность Laravel Boost, добавив собственные рекомендации по использованию ИИ, добавьте файлы `.blade.php` или `.md` в каталог `.ai/guidelines/*` вашего приложения. Эти файлы будут автоматически включены в рекомендации Laravel Boost при выполнении команды `boost:install`.
