...
> [!WARNING]
> По умолчанию стандартная структура приложения Laravel не включает каталог `lang`. Если вы хотите настроить языковые файлы Laravel, вы можете опубликовать их с помощью Artisan-команды `lang:publish`.
...
Методы `afterToday` и `todayOrAfter` можно использовать для гибкого выражения даты и должны означать соответственно «после сегодняшнего дня» или «сегодня или после».
...
Правило проверки `Rule::anyOf` позволяет указать, что проверяемое поле должно удовлетворять любому из заданных наборов правил проверки. Например, следующее правило проверит, что поле `username` является либо адресом электронной почты, либо буквенно-цифровой строкой (включая дефисы), которая имеет длину не менее 6 символов:
...
Чтобы ограничить это правило валидации символами в диапазоне ASCII (`a-z`, `A-Z` и `0-9`), вы можете использовать опцию `ascii`:
...
Методы `beforeToday` и `todayOrBefore` можно использовать для гибкого выражения даты и должны означать соответственно «до сегодняшнего дня» или «сегодня или до».
...
Вы можете использовать параметр `strict`, чтобы считать поле действительным только в том случае, если его значение равно `true` или `false`:
...
Проверяемое поле должно представлять собой массив, содержащий все заданные значения параметров. Поскольку это правило часто требует от вас использовать метод `implode`, метод `Rule::contains` может быть использован для быстрого построения правила:
...
- `rfc`: `RFCValidation` - Проверка адреса электронной почты в соответствии с [поддерживаемыми RFC](https://github.com/egulias/EmailValidator?tab=readme-ov-file#supported-rfcs).
- `strict`: `NoRFCWarningsValidation` - Проверка адреса электронной почты в соответствии с [поддерживаемыми RFC](https://github.com/egulias/EmailValidator?tab=readme-ov-file#supported-rfcs), сбой при обнаружении предупреждений (например, конечных точек и нескольких последовательных точек).
- `dns`: `DNSCheckValidation` - Убедитесь, что домен адреса электронной почты имеет допустимую запись MX.
- `spoof`: `SpoofCheckValidation` - Убедитесь, что адрес электронной почты не содержит омографов или обманчивых символов Unicode.
...
Для удобства правила проверки электронной почты можно создать с помощью текучего конструктора правил:
...
Валидатор `dns` выполняет реальный DNS-запрос, чтобы убедиться, что домен адреса имеет корректную MX-запись. Он не проверяет существование конкретного почтового ящика.

Поскольку тесты не должны зависеть от реальных DNS-запросов, используйте метод `Validator::fakeDnsLookups`, чтобы [подменить DNS-запросы](#rule-active-url), не отключая другие проверки, например `rfc`:
...
Это позволяет приложению продолжать использовать существующие правила валидации во время тестирования:
...
Иногда вам может понадобиться проверить, существует ли массив значений в базе данных. Это можно сделать, добавив правила `exists` и [array](#rule-array) к проверяемому полю:

```php
'states' => ['array', Rule::exists('states', 'abbreviation')],
```

Если оба эти правила назначены полю, Laravel автоматически построит один запрос, чтобы определить, существуют ли все указанные значения в указанной таблице.
...
> [!WARNING]
> Никогда не следует полагаться на валидацию файла только по его назначенному пользователем расширению. Это правило обычно всегда следует использовать в сочетании с правилами [mimes](#rule-mimes) или [mimetypes](#rule-mimetypes).
...
Проверяемый файл должен быть изображением (jpg, jpeg, png, bmp, gif или webp).

> [!WARNING]
> По умолчанию правило изображения не разрешает файлы SVG из-за возможности уязвимостей XSS. Если вам нужно разрешить файлы SVG, вы можете предоставить директиву `allow_svg` правилу `image` (`image:allow_svg`).
...
Проверяемое поле должно быть массивом, имеющим в качестве ключа хотя бы одно из указанных значений:
...
Вы можете использовать параметр `strict`, чтобы считать поле допустимым только в том случае, если его значение имеет целочисленный или плавающий тип. Числовые строки будут считаться недопустимыми:
...
Проверяемое поле должно отсутствовать или быть пустым, если поле _anotherfield_ равно `"yes"`, `"on"`, `1`, `"1"`, `true` или `"true"`.

<a name="rule-prohibited-if-declined"></a>
#### prohibited_if_declined:_anotherfield_,...

Проверяемое поле должно отсутствовать или быть пустым, если поле _anotherfield_ равно `"no"`, `"off"`, `0`, `"0"`, `false` или `"false"`.
...
**Игнорирование мягко удаленных записей в уникальных проверках:**

По умолчанию уникальное правило включает мягко удаленные записи при определении уникальности. Чтобы исключить мягко удаленные записи из проверки уникальности, можно вызвать метод `withoutTrashed`:

```php
Rule::unique('users')->withoutTrashed();
```

Если в вашей модели для мягко удаленных записей используется имя столбца, отличное от `deleted_at`, вы можете указать имя столбца при вызове метода `withoutTrashed`:
...
Проверяемое поле должно быть действительным универсальным уникальным идентификатором (UUID) RFC 9562 (версии 1, 3, 4, 5, 6, 7 или 8).

Вы также можете проверить, соответствует ли указанный UUID спецификации UUID по версии:
...
Если ваше приложение принимает изображения, загружаемые пользователями, вы можете использовать метод конструктора `image` правила `File`, чтобы убедиться, что проверяемый файл является изображением (jpg, jpeg, png, bmp, gif или webp).

Кроме того, для ограничения размеров изображения можно использовать правило `dimensions`:
...
> [!WARNING]
> По умолчанию правило `image` не разрешает файлы SVG из-за возможности уязвимостей XSS. Если вам нужно разрешить файлы SVG, вы можете передать `allowSvg: true` правилу `image`: `File::image(allowSvg: true)`.

<a name="validating-files-image-dimensions"></a>
#### Валидация размеров изображения

Вы также можете проверить размеры изображения. Например, чтобы проверить, что загруженное изображение имеет ширину не менее 1000 пикселей и высоту не менее 500 пикселей, вы можете использовать правило `dimensions`:
