...
В дополнение к методам ответа, перечисленным выше, для определения того, имеет ли ответ определенный код состояния, можно использовать следующие методы:
...
В качестве альтернативы вы можете настроить поведение усечения исключений для каждого запроса, используя метод `truncateExceptionsAt`:
...
В качестве альтернативы вы можете передать массив методу `fake`. Ключи массива должны представлять шаблоны URL, которые вы хотите подделать, и связанные с ними ответы. Допускается использование метасимвола подстановки `*`. Вы можете использовать метод `response` фасада `Http` для создания заглушек / фиктивных ответов для этих адресов:
...
Любые запросы к URL-адресам, которые не были поддельными, будут фактически выполнены. Если вы хотите указать шаблон резервного URL-адреса, который будет заглушать все не сопоставленные URL-адреса, то используйте символ `*`:
...
Чтобы проверить поведение вашего приложения при возникновении исключения `Illuminate\Http\Client\RequestException`, вы можете использовать метод `failedRequest`:
