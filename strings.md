...
Метод `decrypt` [расшифровывает](/docs/{{version}}/encryption) зашифрованную строку:
...
Обратный метод `decrypt` см. в описании метода [encrypt](#method-fluent-str-encrypt).
...
Метод `encrypt` [шифрует](/docs/{{version}}/encryption) строку:
...
Для обратного метода `encrypt` см. метод [decrypt](#method-fluent-str-decrypt).
...
Метод `hash` хеширует строку, используя заданный [алгоритм](https://www.php.net/manual/en/function.hash-algos.php):
...
Метод `toUri` преобразует заданную строку в экземпляр [Illuminate\Support\Uri](/docs/{{version}}/helpers#uri):
