...
Следующие параметры конфигурации относятся только к сторонним поисковым движкам, таким как Algolia, Meilisearch или Typesense. Если вы используете [database engine](#database-engine), этот раздел можно пропустить.
...
После настройки вы можете [определить данные для поиска](#configuring-searchable-data) и начать [выполнять поисковые запросы](#searching) к моделям. В отличие от сторонних движков, database engine не требует отдельного этапа индексации — он ищет напрямую по таблицам базы данных.

#### Настройка стратегий поиска в базе данных

По умолчанию database engine выполняет запрос `LIKE` к каждому атрибуту модели, который вы [настроили для поиска](#configuring-searchable-data). Однако для конкретных столбцов можно назначить более эффективные стратегии. Атрибут `SearchUsingFullText` использует полнотекстовый индекс базы данных, а `SearchUsingPrefix` ищет только начало строк (`example%`) вместо поиска внутри всей строки (`%example%`).
...
Движок «коллекций» предназначен для быстрых прототипов, очень маленьких наборов данных или тестов. Он извлекает все возможные записи из базы данных и фильтрует их в PHP с помощью helper `Str::is`, поэтому не требует индексации или специфичных возможностей базы данных. Для всего, что выходит за рамки тривиальных сценариев, используйте [database engine](#database-engine).
...
После того как вы указали драйвер коллекций в качестве предпочтительного, вы можете начать [выполнение поисковых запросов](#searching) по моделям. Индексирование, необходимое для заполнения индексов Algolia, Meilisearch или Typesense, при использовании этого движка не требуется.
...
Драйвер "collections" является наиболее портативным, так как он работает с любыми реляционными базами данных, поддерживаемыми Laravel (включая SQLite и SQL Server); однако он гораздо менее эффективен, чем драйвер базы данных.
...
Scout позволяет добавлять в поисковые запросы простые условия `where`. В настоящее время эти условия поддерживают только базовые проверки равенства и в первую очередь полезны для ограничения поисковых запросов по идентификатору владельца:
...
При использовании стороннего движка этот callback вызывается после того, как соответствующие модели уже были получены из поискового движка, поэтому его не следует использовать для «фильтрации» результатов — используйте [условия where Scout](#where-clauses). Однако при использовании database engine ограничения метода `query` применяются непосредственно к запросу базы данных, поэтому его можно использовать и для фильтрации.
