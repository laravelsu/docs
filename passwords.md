---
git: 7fd84ae1b51f0961c61ea8d33d34b4d5028b18b8
---
...
Файл конфигурации сброса пароля вашего приложения хранится в `config/auth.php`. Обязательно ознакомьтесь с доступными вам параметрами в этом файле. По умолчанию Laravel настроен на использование драйвера сброса пароля `database`.

Параметр конфигурации сброса пароля `driver` определяет, где будут храниться данные сброса пароля. Laravel включает два драйвера:

<div class="content-list" markdown="1">

- `database` - Данные для сброса пароля хранятся в реляционной базе данных.
- `cache` - Данные для сброса пароля хранятся в одном из ваших кэш-хранилищ.
...
При использовании драйвера `database` по умолчанию необходимо создать таблицу для хранения токенов сброса пароля вашего приложения. Обычно это включено в миграцию базы данных `0001_01_01_000000_create_users_table.php` по умолчанию в Laravel.

<a name="cache"></a>
#### Cache

Также доступен драйвер кэша для обработки сбросов паролей, который не требует выделенной таблицы базы данных. Записи зашифрованы по адресу электронной почты пользователя, поэтому убедитесь, что вы не используете адреса электронной почты в качестве ключа кэша где-либо еще в вашем приложении:
...
Чтобы предотвратить сброс данных сброса пароля вызовом `artisan cache:clear`, вы можете дополнительно указать отдельное хранилище кэша с помощью ключа конфигурации `store`. Значение должно соответствовать хранилищу, настроенному в значении конфигурации `config/cache.php`.
...
Чтобы узнать больше об этом методе посредника, обратитесь к [документации посредника TrustHosts](/docs/{{version}}/requests#configuring-trusted-hosts).
...
Метод `reset` возвращает ключ «status». Этот статус может быть переведен с помощью помощников [локализации](/docs/{{version}}/localization) Laravel, чтобы показать пользователю удобное сообщение о статусе его запроса. Перевод статуса сброса пароля определяется языковым файлом `lang/{lang}/passwords.php` вашего приложения. Запись для каждого возможного значения ключа статуса находится в языковом файле `passwords`. Если ваше приложение не содержит каталога `lang`, вы можете создать его, используя команду Artisan `lang:publish`.
...
Если вы используете драйвер `database`, токены сброса пароля, срок действия которых истек, все еще будут присутствовать в вашей базе данных. Однако вы можете легко удалить эти записи с помощью команды Artisan `auth:clear-resets`:
